...
export function RelationshipMapView({
  ...
  const { positionedNodes, bpNode, canvasWidth, canvasHeight } = useMemo(() => {
    ...
    sortedLevels.forEach((level, colIdx) => {
      ...
      // Sort nodes in this level by their parent's Y coordinate (if any)
      nodesInLevel.sort((a, b) => {
        const parentsA = parentMap.get(a.Id) || [];
        const parentsB = parentMap.get(b.Id) || [];
        const parentPosA = parentsA.map((pId) => posMap.get(pId)).filter(Boolean)[0];
        const parentPosB = parentsB.map((pId) => posMap.get(pId)).filter(Boolean)[0];

        // Invoices often branch upward, Deliveries continue lower
        if (a.ObjType === 13 && b.ObjType === 15) return -1;
        if (a.ObjType === 15 && b.ObjType === 13) return 1;
      ...
      nodesInLevel.forEach((node, rowIdx) => {
        ...
        const parentNode = parents.map((pId) => posMap.get(pId)).filter(Boolean)[0];

        if (totalCount === 1) {
          // If single node in column and has parent, align directly with parent Y
          if (parentNode) {
            y = parentNode.y;
          } else {
            y = BASE_Y;
          ...
          // Multiple nodes in column - stagger vertically
          const totalHeight = totalCount * CARD_HEIGHT + (totalCount - 1) * ROW_GAP;
          const startY = Math.max(
            (parentNode ? parentNode.y : BASE_Y) - totalHeight / 2 + CARD_HEIGHT / 2,
            70
          );
          y = startY + rowIdx * (CARD_HEIGHT + ROW_GAP);
        }
        ...
        posMap.set(node.Id, pNode);

        if (x + CARD_WIDTH + 140 > maxRight) maxRight = x + CARD_WIDTH + 140;
        if (y + CARD_HEIGHT + 140 > maxBottom) maxBottom = y + CARD_HEIGHT + 140;
      });
    });
    ...
    return {
      ...
      bpNode: bp,
      canvasWidth: Math.max(maxRight, 1100),
      canvasHeight: Math.max(maxBottom, 600),
    };
  }, [data]);
  ...
  return (
    <div className="w-full h-full min-h-[calc(100vh-170px)] flex flex-col bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden animate-in fade-in duration-200">
      {/* Top Header Bar */}
      <div className="px-5 py-3 border-b border-slate-200 bg-[#1E3A5F] text-white flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          <Button
            ...
            size="sm"
            onClick={onClose}
            className="h-8 px-2.5 text-white/90 hover:text-white hover:bg-white/10 flex items-center gap-1.5"
            title="Back to Document"
          >
            <ArrowLeft className="h-4 w-4" />
            <span className="text-xs font-semibold">Back to Document</span>
          </Button>
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1 bg-white/10 rounded px-1.5 py-0.5">
            ...
            <span className="text-[11px] font-semibold text-white/90 w-10 text-center select-none">
              {Math.round(zoom * 100)}%
            </span>
            <Button
              type="button"
            ...
            <Button
              ...
              variant="ghost"
              size="icon"
              className="h-6 w-6 text-white/80 hover:text-white hover:bg-white/10 ml-0.5"
      ...
      <div
        ...
        className={`relative flex-1 w-full h-full min-h-[500px] overflow-auto bg-[#FFFFFF] select-none p-6 ${
      ...
      >
        {isLoading ? (
          <div className="flex flex-col items-center justify-center h-full min-h-[400px] gap-2 text-slate-500">
          ...
          <div className="flex flex-col items-center justify-center h-full min-h-[400px] text-slate-400 gap-2">
            ...
            >
              ...
              {(data.Edges || []).map((edge, idx) => {
                ...
                const markerId = isPaymentLink
                  ? "url(#sap-arrow-gold-view)"
                  : "url(#sap-arrow-blue-view)";

                const dx = endX - startX;

                // Clean straight arrow if aligned, smooth curve if offset
                const pathD =
      ...
      <div className="px-4 py-2 border-t border-slate-200 bg-[#F1F5F9] flex items-center justify-between text-xs text-slate-600 shrink-0">
        <div className="flex items-center gap-3">
          <span className="font-semibold text-slate-700">View:</span>
          <span className="bg-white border border-slate-300 rounded px-2.5 py-1 text-xs text-slate-800 font-medium">
