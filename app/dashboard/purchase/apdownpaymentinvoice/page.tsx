...
import {
  patchAPDownPaymentInvoice,
  postAPDownPaymentInvoice,
...
export default function APDownPaymentInvoicePage() {
  const today = new Date().toISOString().split("T")[0];
  const [defaultValues] = useState<APDownPaymentInoviceFormData>({
    CardName: "",
    CardCode: "",
    ContactPersonCode: "",
    DocDate: today,
  ...
  const handleSubmit = async (data: APDownPaymentInoviceFormData) => {
    const {
      lines,
      DocEntry,
      lastLoadedDocType,
    ...
    const payload = {
      ...
      DocumentLines: lines.map((line) => {
        ...
        if (
          ...
          Number(DocEntry) > 0 &&
          lastLoadedDocType &&
          lastLoadedDocType !== DocumentType.APDownPaymentInvoice
        ) {
    ...
    console.log(
      "Final AP Down Payment Invoice Payload:",
    ...
    if (
      ...
      try {
        await patchAPDownPaymentInvoice(Number(DocEntry), payload);
        const docNum = usePurchaseDocument.getState().DocNum;
        toast.success(
          `APDownPaymentInvoice #${docNum || DocEntry} updated successfully`,
        );
      } catch (error) {
        toast.error("Failed to update AP Down Payment Invoice");
      }
      return;
    }
    ...
    try {
      ...
      toast.error(
        message ||
          "Failed to create AP Down Payment Invoice. Please try again.",
      );
      throw error;
    }
  };
  ...
  return (
    <>
      <PurchaseDocumentLayout
        schema={APDownPaymentInvoiceSchema}
        defaultValues={defaultValues}
        onSubmit={handleSubmit}
        docType={DocumentType.APDownPaymentInvoice}
      >
        <div className="flex flex-col gap-6">
          <PurchaseVendorHeader docType={DocumentType.APDownPaymentInvoice} />
          <PurchaseItems />
          <UDFLayout docType={DocumentType.APDownPaymentInvoice} />
          <PurchaseFooter />
        </div>
      </PurchaseDocumentLayout>
    </>
