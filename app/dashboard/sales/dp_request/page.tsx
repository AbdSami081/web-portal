...
export default function ARDownPaymentRequestPage() {
  ...
  const handleSubmit = async (data: QuotationFormData) => {
    ...
    if (DocEntry && Number(DocEntry) > 0 && lastLoadedDocType === DocumentType.DownPaymentRequest) {
      ...
      try {
        ...
        toast.success(`A/R Down Payment Request #${docNum || DocEntry} updated successfully`);
      } catch (error) {
        const message = getSapErrorMessage(error);
        toast.error(message || "Failed to update A/R Down Payment Request");
        throw error;
      }
      return;
    }
    ...
    const payload = buildSalesDocumentPayload({
      ...
      docEntry: DocEntry,
      lastLoadedDocType,
      targetDocType: DocumentType.DownPaymentRequest,
      discountPercent,
      freight,
      additionalExpenses,
      downPaymentType: "dptRequest",
    ...
    try {
      ...
      if (response?.DocEntry || response?.IsDraft) {
        ...
        if (response?.IsDraft) {
          toast.success("A/R Down Payment Request submitted for approval.");
        } else {
      ...
      const message = getSapErrorMessage(error);
      toast.error(message || "Failed to create A/R Down Payment Request. Please try again.");
      throw error;
    }
  };
  ...
  return (
    <SalesDocumentLayout
      ...
      docType={DocumentType.DownPaymentRequest}
    >
      <DocumentHeader />
      <DocumentItems />
      <UDFLayout docType={DocumentType.DownPaymentRequest} />
      <DocumentFooter />
    </SalesDocumentLayout>
  );
}
